...
#container, add a user account (and make them sudo), set up the IP address, 
#and set up the permissions on the jumpbox for access.
...
if sudo lxc info $1:$2 2>&1 | grep -q 'error'; then
    echo "Container $1:$2 not found"
    exit 1
fi
